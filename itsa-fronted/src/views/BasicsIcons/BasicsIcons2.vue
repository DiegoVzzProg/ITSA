...
const handleScroll = () => {
    if (contenedor_scroll.value && contenedor_children_scroll.value && padre_contenedor_scroll.value) {
        const rect = contenedor_scroll.value.getBoundingClientRect();
        const rect2 = contenedor_children_scroll.value.getBoundingClientRect();
        const scrollTop = window.scrollY || document.documentElement.scrollTop;
        const topAbsoluto = rect.top + scrollTop;
        if (padre_contenedor_scroll.value.getBoundingClientRect().height / 2 <= (topAbsoluto + 50)) {
            children_elemento1_scroll.value?.classList.add('hidden');
            children_elemento1_scroll.value?.classList.remove('flex');
            children_elemento2_scroll.value?.classList.remove('hidden');
            children_elemento2_scroll.value?.classList.add('flex');
        } else {
            children_elemento1_scroll.value?.classList.add('flex');
            children_elemento1_scroll.value?.classList.remove('hidden');
            children_elemento2_scroll.value?.classList.add('hidden');
            children_elemento2_scroll.value?.classList.remove('flex');
...
<template>
    <div class="flex flex-col min-h-screen w-full grow shrink-0 top-0 left-0" ref="padre_contenedor_scroll">
        <div class="flex fixed top-0 left-0 h-screen bg-[rgba(183,164,237,0.45)] transition-all w-full" id="overflow">
        </div>
        <div class="w-full min-h-screen flex justify-center items-center sticky top-0">
            <div ref="contenedor_scroll" class="flex flex-col w-full justify-start items-center">
                <div ref="children_elemento1_scroll"
                ...
                <div ref="children_elemento2_scroll"
                    class="hidden animate-fade-in flex-col w-full justify-center items-center">
                    <p
                        class="font-itsa-bold text-[clamp(35px,10vw,208px)] leading-[78px] font-bold flex text-black z-10 select-none">
                        +50 elements
                    </p>
                    <p
                        class="font-itsa-bold text-[clamp(20px,7vw,108px)] max-[480px]:translate-y-[-20px] text-center font-bold flex text-black z-10 select-none">
                        personal & comercial use
                    </p>
                </div>
            </div>
        </div>
        <div class="w-full flex min-h-screen justify-center items-center" ref="contenedor_children_scroll">
        </div>
    </div>
