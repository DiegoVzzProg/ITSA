<template>
  <div
    class="flex w-full min-h-full grow shrink-0 flex-col justify-center items-center gap-3"
  >
    <div
      class="flex w-full justify-end max-w-md sticky top-[-1px] bg-white py-2"
    >
    ...
    <div
      class="flex flex-row w-full justify-center max-w-screen-lg items-center gap-3 max-[1020px]:flex-col"
    >
      <Form
        @submit="btnRegisterUser_OnClick"
        :validation-schema="validationSchema"
        class="flex w-full max-w-md flex-col justify-between gap-3 h-[min(500px,100%)]"
        v-if="ContinueRegistration == 0"
      >
        <p class="px-[clamp(18px,3vw,28px)]">create a new user</p>
        <div
          class="flex flex-col gap-1"
          v-for="(item, index) in FormRegister.User"
          :key="index"
        >
          <Field
            :name="item.id"
        ...
        <div class="flex flex-col gap-2 w-full ps-5">
          ...
          <span
            class="text-[rgb(216,70,70)] text-sm font-semibold"
            v-if="FormLeyoTerms.error"
          >
            {{ FormLeyoTerms.error }}
          </span>
        </div>
