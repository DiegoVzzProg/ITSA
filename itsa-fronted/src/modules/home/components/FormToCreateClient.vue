        ...
        :initial-values="GetRecords()">
            <div class="flex flex-col gap-2 w-full">
                ...
                <div class="flex flex-col gap-1">
                    <Field :name="FormRegister.Customer.Form3.country.id" v-slot="{ field }">
                        <SelectCountry :model-value="field.value" @update:model-value="field.onChange"
            ...
            <div class="flex flex-col w-full max-w-screen-lg max-[1020px]:max-w-md">
                ...
                <button  v-else-if="!loading && !ClientData" type="submit"
...
import { CostumersClass, IEditCustomer, IRegisterClient } from '../services/costumers-service';

const ClientData = ref<Record<string, any> | undefined>(undefined)
const RegexVatNumber = ref<RegExp>(/^(?:(?:[A-Z&Ñ]{3,4}\d{2}(?:0[1-9]|1[0-2])(?:0[1-9]|[12]\d|3[01])[A-Z\d]{3})|(?:ATU\d{8}|BE0\d{9}|BG\d{9,10}|HR\d{11}|CY\d{8}[A-Z]|CZ\d{8,10}|DE\d{9}|DK\d{8}|EE\d{9}|EL\d{9}|ES[A-Z]\d{7}[A-Z]|FI\d{8}|FR[A-HJ-NP-Z0-9]{2}\d{9}|GB(?:\d{9}|\d{12}|GD\d{3}|HA\d{3})|HU\d{8}|IE\d{7}[A-W]|IT\d{11}|LT(?:\d{9}|\d{12})|LU\d{8}|LV\d{11}|MT\d{8}|NL\d{9}B\d{2}|PL\d{10}|PT\d{9}|RO\d{2,10}|SE\d{12}|SI\d{8}|SK\d{10}))$/);
    ...
    Customer: {
        Form1: {
            name: {
                ...
                .matches(/^[a-zA-Z\u00C0-\u00FF\s]+$/, 'the input can only contain letters'),
                type: "text"
            },
            vat_number: {
        ...
        Form2: {
            postal_code: {
                ...
                .matches(/^[A-Za-z0-9\s\-]+$/, 'postal code contains invalid characters'),
                type: "text"
            },
            state: {
                id: "estado",
...
async function btnFunctionEdit(data: any) {
    ...
    const params: IEditCustomer = {
        ...
        numero_de_iva_empresa: data.vat_number.value,
        direccion: data.address.value,
        codigo_postal: data.postal_code.value,
        estado: data.state.value,
        id_pais: data.country.id_pais,
...
async function btnFunctionAdd(data: any){
    ...
    const response: ApiResponse = await new CostumersClass().registerClient(dataI);
