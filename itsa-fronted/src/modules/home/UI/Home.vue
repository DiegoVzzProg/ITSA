...
<template>
    <div class="flex flex-col grow shrink-0 min-h-screen w-full">
        <div class="min-h-screen flex flex-col pt-[76px] justify-center items-center" ref="section1">
            <div class="flex flex-col max-h-[40px] overflow-hidden text-center">
                <span v-for="(item, index) in texts" :key="index"
                    :class="[item.class, 'text-[clamp(2.7rem,4vw,3rem)] animate-scroll-down font-itsa-bold translate-y-[-10px]']">
                    {{ item.texto }}
                </span>
            </div>
            <p class="text-[clamp(1.5rem,3vw,3rem)] text-center"
        ...
        <div id="gallery"
            ...
            <div class="flex flex-row-reverse w-full justify-end gap-3 py-5 transition-all flex-wrap max-[1024px]:justify-center"
                v-if="productos">
                <div v-for="(producto, index) in productos" :key="index" :class="[
                    ...
                    'bg-[rgb(244,242,239)] rounded-[12px] cursor-pointer p-5 flex-1']">
